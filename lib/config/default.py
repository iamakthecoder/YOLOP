...
_C.LOG_DIR = 'runs/'
_C.GPUS = (0,1)     
_C.WORKERS = 2 #reduced from 8 to 2
_C.PIN_MEMORY = False
_C.PRINT_FREQ = 20
_C.AUTO_RESUME = True       # Resume from the last training interrupt
_C.NEED_AUTOANCHOR = False      # Re-select the prior anchor(k-means)    When training from scratch (epoch=0), set it to be true!
_C.DEBUG = False
_C.num_seg_class = 2
...
_C.TRAIN.VAL_FREQ = 1
_C.TRAIN.BATCH_SIZE_PER_GPU =16 #reduced from 24 to 16
...
_C.TEST.BATCH_SIZE_PER_GPU = 16 #reduced from 24 to 16
_C.TEST.MODEL_FILE = ''
